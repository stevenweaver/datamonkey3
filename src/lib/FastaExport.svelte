<script>
	...
	function generateCsvText(sequences) {
		...
		for (const seq of sequences) {
			const row = [`"${seq.name}"`, seq.partition, seq.sequence.length, `"${seq.sequence}"`];
	...
	<div class="fasta-export mb-4 rounded-lg border border-border-subtle bg-white p-4 shadow-sm">
			...
			{#if exportFormat === 'fasta'}
				<div>
					...
					<div class="flex flex-wrap gap-3">
						<label class="flex cursor-pointer items-center">
							<input type="checkbox" bind:checked={includePadding} class="mr-1" />
		...
		<div class="mt-3 text-xs text-text-silver">
			...
			{:else if exportFormat === 'csv'}
				<p>
					Exports sequences in CSV format with additional metadata including partition and sequence
					length.
				</p>
			{:else}
