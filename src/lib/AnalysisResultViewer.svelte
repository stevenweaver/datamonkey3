    ...
    <div class="analysis-container">
      ...
      <div class="bg-gray-100 p-4">
        ...
        <div class="text-sm text-gray-600">
          ...
          <p>Status: 
            <span class="font-semibold capitalize {analysis.status === 'completed' ? 'text-green-600' : analysis.status === 'pending' ? 'text-yellow-600' : 'text-gray-600'}">
        ...
        {#if analysis.status === 'completed' && resultData}
          ...
          {:else if ['FEL', 'SLAC', 'MEME', 'BUSTED', 'FUBAR'].includes(analysis.method)}
            ...
            <div class="selection-analysis">
              ...
              {#if analysis.method === 'FEL'}
                <div class="mt-6 mb-6 bg-white p-4 rounded-lg shadow-sm">
                  <h3 class="text-lg font-semibold mb-4">FEL Analysis Visualization</h3>
                ...
                <iframe 
                  ...
                  on:load={(e) => {
                    ...
                    if (e.target.contentWindow) {
                      e.target.contentWindow.postMessage({ type: 'json', data: resultData }, '*');
                    }
                  }}
                ></iframe>
              </div>
                    ...
                    <tbody>
                      {#each resultData.fits as fit}
                        <tr class="border-b">
                          ...
                          <td class="p-2">{fit.log_likelihood ? fit.log_likelihood.toFixed(2) : 'N/A'}</td>
          ...
          {:else}
            ...
            <div class="json-display">
              ...
              <pre class="bg-gray-100 max-h-96 overflow-auto p-2 text-sm">{JSON.stringify(resultData, null, 2)}</pre>
            </div>
          {/if}
        {:else if ['pending', 'running', 'mounting', 'processing', 'saving'].includes(analysis.status)}
          <div class="flex flex-col items-center justify-center p-4">
            <div class="loader mb-4"></div>
            <p class="text-yellow-600 text-lg">Analysis is {analysis.status === 'pending' ? 'pending...' : analysis.status + '...'}</p>
            <p class="text-sm text-gray-500 mt-2">This might take a few moments to complete.</p>
            <div class="flex justify-center mt-4">
              <div class="w-20 h-20">
                <svg class="animate-spin" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                  <circle cx="12" cy="12" r="10" stroke-width="4" stroke="currentColor" stroke-opacity="0.25" fill="none" />
