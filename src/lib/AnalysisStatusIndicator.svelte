<script>
  ...
  // Derived counts from activeAnalyses
  ...
  $: failedCount = $activeAnalyses ? 
    $activeAnalyses.filter(a => a.status === 'error').length : 0;
...
{#if showIndicator}
  <button 
    class="flex items-center space-x-2 rounded-full bg-white px-3 py-1.5 shadow-sm border border-gray-200 hover:shadow-md transition-shadow duration-200"
    ...
    {#if runningCount > 0}
      <div class="flex items-center">
        <span class="inline-flex items-center justify-center text-sm">
          <span class="mr-1 h-2 w-2 rounded-full bg-blue-500 pulse-animation"></span>
