...
class AnalysisRunner {
    constructor() {
        ...
        this.unsubscribe = analysisQueue.subscribe(queue => {
            ...
            const pendingAnalyses = queue.filter(analysis => analysis.status === AnalysisStatus.PENDING);
        ...
        try {
            ...
            const result = await getCachedOrCompute(
                ...
                async () => {
                    ...
                    // Mount the file
                    const inputFiles = await cliObj.mount([
                        { name: 'user.nex', data: fileData }
                    ]);
                    ...
                    for (const [key, value] of Object.entries(analysis.config)) {
                        if (key !== 'tree') { // Skip the tree as it's handled separately
                    ...
                    if (stdout.includes('is not a valid choice passed to')) {
                        ...
                        if (errorMatch) {
                            ...
                            throw new Error(`HyPhy error: '${invalidValue}' is not a valid value for parameter '${paramName}'. Please check your input parameters.`);
                        } else {
                    ...
                    // Get the JSON result
                    const jsonBlob = await cliObj.download(`/shared/data/user.nex.${analysis.method.toUpperCase()}.json`);
            ...
            console.log(`Analysis ${analysis.method} completed successfully${result.cached ? ' (from cache)' : ''}`);
        } catch (error) {
