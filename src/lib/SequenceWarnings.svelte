  ...
  function generateWarnings(data) {
    ...
    if (data.FILE_INFO.duplicate_sequences && data.FILE_INFO.duplicate_sequences > 0) {
      warnings.push({
        ...
        details: 'Duplicate sequences can affect the accuracy of selection analysis. Consider removing duplicates for more reliable results.',
    ...
    if (data.FILE_INFO.sites && data.FILE_INFO.sites < 30) {
      warnings.push({
        ...
        details: 'Short alignments may have limited statistical power for detecting selection. Consider adding more sequences or sites if possible.',
        action: null
      });
    }
    ...
    if (data.FILE_INFO.sequences && data.FILE_INFO.sequences < 8) {
      warnings.push({
        ...
        details: 'Selection analyses generally perform better with larger samples. Consider adding more sequences if possible.',
        action: null
      });
    }
    ...
    if (data.FILE_INFO.ambiguous_sites && data.FILE_INFO.ambiguous_sites > 0) {
      warnings.push({
        ...
        details: 'Ambiguous characters (like N, X, etc.) may cause issues with some analyses. Consider resolving these ambiguities if possible.',
        action: null
      });
    }
    ...
    if (data.FILE_INFO.stop_codons && data.FILE_INFO.stop_codons > 0) {
      warnings.push({
        ...
        details: 'Stop codons within the alignment may indicate frame shifts, sequencing errors, or pseudogenes. Verify your alignment and genetic code selection.',
        action: null
      });
    }
...
{#if warnings.length > 0}
  <div class="sequence-warnings mt-4 rounded-lg border border-gray-200 bg-white p-4 shadow-sm">
    <div class="flex items-center justify-between">
      ...
      {#if warnings.length > 2}
        <button 
          ...
          on:click={toggleShowAllWarnings}
        >
    ...
    <div class="mt-4 text-xs text-gray-500">
      <p>These warnings are meant to help you improve your analysis results. Minor issues may not significantly affect results.</p>
    </div>
  </div>
{/if}