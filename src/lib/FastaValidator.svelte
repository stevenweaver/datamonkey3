<script>
  ...
  function getSeverityClass(isError) {
    return isError ? 'bg-red-100 border-red-400 text-red-700' : 'bg-yellow-100 border-yellow-400 text-yellow-700';
  }
...
<div class="fasta-validator mb-6 rounded-lg border border-gray-200 bg-white p-4 shadow-sm">
  ...
  {:else if validationResults}
    ...
    <div class="mb-4 rounded border border-gray-200 p-3">
      ...
      <div class="mb-3 rounded p-2" class:bg-green-100={validationResults.valid} class:bg-red-100={!validationResults.valid}>
        <p class="font-medium" class:text-green-700={validationResults.valid} class:text-red-700={!validationResults.valid}>
          {validationResults.valid ? 'Valid FASTA file' : 'Invalid FASTA file'}
        </p>
        {#if validationResults.stats}
          <p class="text-sm">
            <span class="font-medium">Sequences:</span> {validationResults.stats.sequenceCount}
            {#if validationResults.stats.sequenceCount > 0}
              <span class="mx-1">|</span>
              <span class="font-medium">Type:</span> {validationResults.stats.detectedType === 'nucleotide' ? 'Nucleotide' : 'Amino Acid'}
              <span class="mx-1">|</span>
          ...
          <ul class="max-h-48 overflow-y-auto">
            ...
            {#each validationResults.warnings as warning}
              <li class="mb-1 rounded border-l-4 border-yellow-400 bg-yellow-50 p-2 text-sm">
                <div class="flex items-start justify-between">
                  <div>
                    <span class="font-mono text-xs text-yellow-800">{formatErrorCode(warning.type)}</span>
                    <span class="ml-2 font-medium">{warning.type ? warning.type.message : 'Warning'}</span>
                  </div>
                </div>
            ...
            <div class="mb-3">
              ...
              <ul class="ml-4 list-disc text-sm">
                ...
                {#if repairedData.repairs.length > 5}
                  <li class="text-gray-500">...and {repairedData.repairs.length - 5} more repairs</li>
                {/if}
              </ul>
            </div>
          {/if}
