    ...
    try {
      ...
      if (['fel', 'slac', 'meme', 'fubar'].includes(analysis.method)) {
        if (result.tested && result.tested.sites) {
          ...
          const positiveSites = result.tested.sites.filter(site => 
            site.p <= 0.05 && (site.beta > site.alpha || site.posterior >= 0.9)
          ).length;
...
<div 
  class="analysis-card mb-3 rounded-lg border transition-all duration-200 {compact ? 'p-3' : 'p-4'} {selected ? 'border-blue-500 bg-blue-50 shadow-sm' : 'border-gray-200 bg-white hover:shadow-sm hover:border-gray-300'}"
  on:click={selectCard}
>
  <div class="flex items-start">
    <!-- Method and status -->
    <div class="mr-3 {compact ? 'w-8 h-8' : 'w-10 h-10'} flex-shrink-0 rounded-md bg-blue-100 text-blue-600 flex items-center justify-center">
    ...
    <div class="flex-grow">
      ...
      <div class="flex justify-between items-start">
        ...
        <!-- Status badge -->
        <div class="{compact ? 'text-xs' : 'text-sm'} px-2 py-0.5 rounded-full inline-flex items-center"
          ...
          class:bg-gray-100={!['completed', 'running', 'pending', 'error'].includes(analysis.status)}
          class:text-gray-800={!['completed', 'running', 'pending', 'error'].includes(analysis.status)}
        >
          {#if analysis.status === 'running' || analysis.status === 'pending'}
            <svg class="animate-spin -ml-0.5 mr-1.5 h-2 w-2" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
          ...
          {/if}
          <span class="capitalize">{analysis.status === 'completed' ? 'Completed' : analysis.status || 'unknown'}</span>
        </div>
      </div>
      
      <!-- Info -->
      <div class="text-gray-500 {compact ? 'text-xs mt-0.5' : 'text-sm mt-1'}">
      ...
      {#if !compact && resultPreview}
        <div class="mt-3 p-2 rounded bg-gray-50 border border-gray-100 text-sm">
          {#if resultPreview.type === 'selection'}
            <div class="text-purple-700 font-medium">
              {resultPreview.text}
            </div>
          {:else if resultPreview.type === 'hypothesis'}
            <div class="{resultPreview.significant ? 'text-green-700' : 'text-orange-700'} font-medium">
              {resultPreview.text}
            </div>
          {:else}
  ...
  {#if !compact}
    <div class="actions mt-3 flex justify-end gap-2">
      <button 
        ...
        class="inline-flex items-center rounded px-2.5 py-1.5 text-xs font-medium bg-blue-100 text-blue-700 hover:bg-blue-200 transition-colors"
      >
        <svg xmlns="http://www.w3.org/2000/svg" class="mr-1 h-3 w-3" viewBox="0 0 20 20" fill="currentColor">
      ...
      <button 
        ...
        class="inline-flex items-center rounded px-2.5 py-1.5 text-xs font-medium bg-green-100 text-green-700 hover:bg-green-200 transition-colors"
      >
        <svg xmlns="http://www.w3.org/2000/svg" class="mr-1 h-3 w-3" viewBox="0 0 20 20" fill="currentColor">
      ...
      <button 
        ...
        class="inline-flex items-center rounded px-2.5 py-1.5 text-xs font-medium bg-purple-100 text-purple-700 hover:bg-purple-200 transition-colors"
      >
        <svg xmlns="http://www.w3.org/2000/svg" class="mr-1 h-3 w-3" viewBox="0 0 20 20" fill="currentColor">
