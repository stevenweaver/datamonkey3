<script>
	...
	$: filteredAnalyses = analyses.filter(
		(a) => analysisTypeFilter === 'all' || a.method === analysisTypeFilter
	);
	...
	async function compareAnalyses() {
		...
		try {
			...
			// Parse result JSON
			const result1 =
				typeof analysis1.result === 'string' ? JSON.parse(analysis1.result) : analysis1.result;

			const result2 =
				typeof analysis2.result === 'string' ? JSON.parse(analysis2.result) : analysis2.result;
	...
	function generateComparisonData(method, result1, result2, analysis1, analysis2) {
		...
		const file1 =
			$persistentFileStore.files.find((f) => f.id === analysis1.fileId)?.filename || 'Unknown';
		const file2 =
			$persistentFileStore.files.find((f) => f.id === analysis2.fileId)?.filename || 'Unknown';
				...
				if (result1.tested?.sites && result2.tested?.sites) {
					...
					comparison.summary = {
						file1: {
							...
							positiveCount:
								result1.tested.sites.filter((s) => s.p <= 0.05 && s.beta > s.alpha).length || 0
						},
						file2: {
							sites: result2.tested.sites.length || 0,
							positiveCount:
								result2.tested.sites.filter((s) => s.p <= 0.05 && s.beta > s.alpha).length || 0
						}
					};
					...
					result1.tested.sites.forEach((site1) => {
						...
						if (site2) {
							...
							const status1 =
								site1.p <= 0.05 && site1.beta > site1.alpha
									? 'positive'
									: site1.p <= 0.05 && site1.beta < site1.alpha
										? 'negative'
										: 'neutral';
							const status2 =
								site2.p <= 0.05 && site2.beta > site2.alpha
									? 'positive'
									: site2.p <= 0.05 && site2.beta < site2.alpha
										? 'negative'
										: 'neutral';
					...
					result2.tested.sites.forEach((site2) => {
						...
						const site1Index = result1.tested.sites.findIndex(
							(s) => (s.site_index || s.site) === siteIndex
						);
...
<div class="analysis-compare rounded-lg border border-gray-200 bg-white p-4 shadow-sm">
	...
	<div class="mb-4">
		...
		<p class="mb-2 text-sm text-gray-600">
			Select up to {maxSelections} analyses of the same type to compare. For the most meaningful comparison,
			select analyses with similar parameters.
		</p>
		...
		<div class="max-h-48 overflow-y-auto rounded border border-gray-200">
			{#if filteredAnalyses.length === 0}
				<p class="p-3 text-center text-sm text-gray-500">
					No analyses available{analysisTypeFilter !== 'all'
						? ` for type ${analysisTypeFilter.toUpperCase()}`
						: ''}
				</p>
			{:else}
					...
					<tbody>
						{#each filteredAnalyses as analysis}
							...
							>
								...
								<td class="truncate p-2">{file?.filename || 'Unknown file'}</td>
		...
		<div class="comparison-results mt-4 rounded border border-gray-200 p-4">
			...
			{#if comparisonData.siteComparisons && comparisonData.siteComparisons.length > 0}
				<div class="site-comparisons">
					...
					<p class="mb-2 text-sm text-gray-600">
						Showing only sites with significant differences in selection status.
					</p>
							...
							<tbody>
								{#each comparisonData.siteComparisons.filter((s) => s.changed) as comparison}
									<tr class="border-t border-gray-200">
										...
										<td class="p-2"
											>{comparison.p1 !== null ? comparison.p1.toExponential(2) : '-'}</td
										>
										<td class="p-2"
											>{comparison.p2 !== null ? comparison.p2.toExponential(2) : '-'}</td
										>
										<td class="p-2">
											...
											{:else if comparison.status1 === 'missing'}
												<span class="rounded bg-gray-100 px-1.5 py-0.5 text-xs text-gray-800"
													>Not Present</span
												>
											{:else}
	...
	{:else if selectedAnalyses.length === 2}
		<div class="mt-4 rounded bg-yellow-100 p-3 text-sm text-yellow-800">
			To compare analyses, select two analyses of the same type. Currently, you have selected
			different types.
		</div>
	{/if}
</div>
