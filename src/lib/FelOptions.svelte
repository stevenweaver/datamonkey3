...
<div id="fel-options" class="rounded-lg bg-white p-6 shadow-md">
	...
	<div class="mb-4">
		<label id="geneticcode-content" class="mb-1 block text-text-slate">
			Genetic Code <a href="/help#genetic-code" target="_blank" class="text-brand-royal"
				><sup>?</sup></a
			>
		</label>
		<select
	...
	<div class="mb-4">
		<label class="mb-1 block text-text-slate">Synonymous rate variation (recommended)?</label>
		<select
	...
	<div class="mb-4">
		<label for="multiple-hits" class="mb-1 block text-text-slate">Multiple Hits</label>
		<select
	...
	<div class="mb-4">
		<label for="estimated-rates" class="mb-1 block text-text-slate">Site Multihit</label>
		<select
	...
	<div class="mb-6">
		...
		<div class="mb-4">
			<label class="mb-1 block text-text-slate">Selection Mode</label>
			<select bind:value={branchesToTest} class="w-full rounded border border-border-subtle p-2">
		...
		{#if branchesToTest === 'Custom'}
			<div class="mb-4">
				<label class="mb-1 block text-text-slate">Branch Names (comma-separated)</label>
				<input
					type="text"
					...
					class="w-full rounded border border-border-subtle p-2"
				/>
				<p class="mt-1 text-sm text-text-silver">Or use a regex pattern:</p>
				<input
					type="text"
					bind:value={regexPattern}
					placeholder="e.g. /^human/i for case-insensitive match"
					class="mt-1 w-full rounded border border-border-subtle p-2"
				/>
			</div>
		{/if}
		...
		{#if branchesToTest === 'Interactive' && selectedTreeData}
			<div class="mb-4">
				<p class="mb-2 text-sm text-text-slate">
					Click on branches in the tree below to select them for testing:
				</p>
				<div class="rounded border border-border-subtle p-4">
					<BranchSelector
		...
		{#if taggedNewick && branchesToTest !== 'All'}
			<div class="mb-4">
				<label class="mb-1 block text-text-slate">Tagged Tree Preview</label>
				<textarea
					value={taggedNewick}
					readonly
					rows="3"
					class="w-full rounded border border-border-subtle bg-surface-raised p-2 font-mono text-sm"
				></textarea>
				<p class="mt-1 text-xs text-text-silver">
	...
	<div class="mt-6">
		...
		<div class="mt-2 rounded-md border border-border-subtle bg-surface-raised p-4">
			<div class="mb-4">
				<p class="text-sm text-text-slate">
				...
				<label class="mb-1 block text-text-slate">Resample?</label>
				<input
					type="number"
					bind:value={resample}
					placeholder="50"
			...
			<div class="mb-4">
				...
				<label class="inline-flex items-center">
					<input
						...
						class="form-checkbox rounded text-brand-royal focus:ring-brand-muted"
					/>
